...
function findColorReferences(content) {
	...
	const variantPatterns = cssVariants
		.map((variant) => `${variant}\\\\?:`)
		.join("|");
	...
	for (const [oldColor] of Object.entries(colorMapping)) {
		...
		const classRegex = new RegExp(
			`(\\s|"|'|:)(((?:${variantPatterns})?)((?:${prefixPatterns})-)(${oldColor}))(\\s|"|'|\\)|\\}|/)`,
			"g",
		);
	...
	try {
		...
		if (verbose) {
			...
			references.forEach((ref) => {
				console.log(
					`  ${ref.prefix} -> ${ref.utility}${colorMapping[ref.color]}`,
				);
			});
		}
		...
		const _offset = 0; // Track position offset as we make replacements
		...
		if (content !== newContent) {
			if (!dryRun) {
				...
				console.log(
					`🔍 Would update ${filePath} (${references.length} references)`,
				);
			}
...
async function migrateColors() {
	...
	console.log(
		`Files ${dryRun ? "that would be " : ""}updated: ${results.updated.length}`,
	);
