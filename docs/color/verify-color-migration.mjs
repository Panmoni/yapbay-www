...
const excludeDirs = ["node_modules", ".git", ".vscode", ".astro"];
...
async function _findOldColorReferences(filePath) {
	try {
		...
		for (const oldColor of oldColorPatterns) {
			for (const prefix of cssPrefixes) {
				...
				let match;
				do {
					match = directPattern.exec(content);
					if (match !== null) {
						references.push({
							color: oldColor,
							pattern: match[2],
				...
				for (const variant of cssVariants) {
					const variantPattern = new RegExp(
						`(\\s|"|'|:)(${variant}\\\\?:${prefix}-${oldColor})(\\s|"|'|\\)|\\}|/)`,
						"g",
					);
					do {
						match = variantPattern.exec(content);
						if (match !== null) {
							references.push({
								color: oldColor,
								pattern: match[2],
...
async function scanDirectory(dirPath, results = []) {
	...
	for (const entry of entries) {
		...
		if (entry.isDirectory()) {
			...
			const references = await _findOldColorReferences(fullPath);
...
async function verifyMigration() {
	...
	if (results.length === 0) {
		console.log(
			"✅ No old color references found! The migration was successful.",
		);
	} else {
		...
		for (const result of results) {
			...
			if (verbose) {
				...
				const patterns = [
					...new Set(result.references.map((ref) => ref.pattern)),
				];
		...
		console.log(
			"Run with --verbose for detailed information about each reference.",
		);
	}
}
